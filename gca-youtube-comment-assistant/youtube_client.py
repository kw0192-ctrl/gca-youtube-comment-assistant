...
def get_author_channel_id(comment_snippet):
    return (
        comment_snippet
        .get("authorChannelId", {})
        .get("value", "")
...
def thread_has_reply_from_me(thread_item, my_channel_id):
    replies = thread_item.get("replies", {}).get("comments", [])

    for reply in replies:
        reply_snippet = reply.get("snippet", {})
        reply_author_id = get_author_channel_id(reply_snippet)

        if reply_author_id == my_channel_id:
            return True
...
def fetch_recent_comments(youtube, max_results=25):
    ...
    for item in response.get("items", []):
        ...
        top_author_id = get_author_channel_id(top)

        if top_author_id == channel_id:
            continue

        if thread_has_reply_from_me(item, channel_id):
            continue
